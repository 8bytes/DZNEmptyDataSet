...
- (UIView *)dzn_buttonView
{
    if (self.emptyDataSetSource && [self.emptyDataSetSource respondsToSelector:@selector(viewForButton:)]) {
        return [self.emptyDataSetSource viewForButton:self];
    }
    return nil;
}
...
- (void)dzn_reloadEmptyDataSet
{
    ...
    if (([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0) || [self dzn_shouldBeForcedToDisplay])
    {
        ...
        UIView *customButtonView = [self dzn_buttonView];
        ...
        if (customView) {
            ...
            }else if (customButtonView){
                view.buttonView = customButtonView;
            }
        }
...
- (void)setupConstraints
{
    ...
    if (_customView) {
        ...
        if (_buttonView) {
            ...
            [subviewStrings addObject:@"buttonView"];
            views[[subviewStrings lastObject]] = _buttonView;
            [self.contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(padding@750)-[buttonView(>=0)]-(padding@750)-|" options:0 metrics:metrics views:views]];
        }
